...
//Database code start
function getDatabaseConnection($state,$district){
    $username="user-313135c319";
	$password="5=h4eLTs<Gyx";
	$database="history-313135c319";
	$mysqli = new mysqli("shareddb-p.hosting.stackcp.net",$username,$password,$database);
    if ($mysqli->connect_errno) {
		$subject="Database Connection Error in ".$state."(".$district.")";
		$msg='<h3 style="text-decoration:underline;">Failed to connect to MySQL: (' . $mysqli->connect_errno . ')</h3><h4 style="color:black;">'.$mysqli->connect_error.'<br>Invalid Credentials...</h4>';
...
function insertDataToDB($con,$data,$state,$district,$date){
	$string_data=addslashes($data);
	$check='SELECT * FROM veg_prices_history where date="'.$date.'"AND state="'.$state.'"AND district="'.$district.'"AND details="'.$string_data.'"';
	if($con!=null){
	    if(($con->query($check))->num_rows==0){
            $sql = 'INSERT INTO veg_prices_history(date,state,district,details) VALUES ("' . $date . '","'.$state.'","'.$district.'","' . $string_data .'")';
            if($con->query($sql) === true){
                //echo "New record inserted<br>";
            }
            else
			{
				$subject="Data Insertion Error in ".$state."(".$district.")";
				$msg='<h4>Error:' /*. $sql . '<br>'*/ . $con->error.'</h4>';
				sendEmail($subject,$msg);
			}
		}
		/*else
	...
	//echo "<br>End of insert";
}
function getDBData($con){
	$sql='SELECT * FROM veg_prices_history';
	$res = $con->query($sql);
	if ($res->num_rows > 0) {
		// output data of each row
        while($row = $res->fetch_assoc()) {
            echo "Date: " . $row["date"] . " , State: " . $row["state"]. " , District: " . $row["district"]. " , JSON : ".$row["details"] ."<br>";
        }
    }
    else {
        echo "Zero results";
...
function getData($api_url,$dates_file,$json_file,$state,$district)
{
	date_default_timezone_set("Asia/Kolkata");
	$date=date("Y-m-d");
	if(file_exists($dates_file) && (in_array($date."\n", file($dates_file)))) //Checks if date_file  and also the date exists in folder
	{
		echo file_get_contents($json_file);  //Returns contents of an existing JSON file
	}
	else
	{
		$status=pricejson($api_url,$json_file,$date,$state,$district); 
		if($status)
		   file_put_contents($dates_file,$date."\n",FILE_APPEND);
	}
}
...
function pricejson($url,$json_file,$date,$state,$district)
{ 
    try 
    {  
    	...
    	$Row_finder=new DomXPath($DOM);
    	$Row_nodes=$Row_finder->query("//*[contains(@class,'Row')]");
    	$json_out=array();
    	foreach($Row_nodes as $node) {
    		...
    		$Cell_nodes=$Cell_finder->query("//*[contains(@class,'Cell')]");
    		$sub_string=array();
    		$sub_string[]=substr(strip_tags(innerHTML($Cell_nodes[0])),0,strlen(strip_tags(innerHTML($Cell_nodes[0])))-1);
    		$sub_string[]=substr(strip_tags(innerHTML($Cell_nodes[2])),9,strlen(strip_tags(innerHTML($Cell_nodes[2])))-10);
    		$sub_string[]=substr(strip_tags(innerHTML($Cell_nodes[3])),9,strlen(strip_tags(innerHTML($Cell_nodes[3])))-10);
    		$sub_string[]=substr(strip_tags(innerHTML($Cell_nodes[4])),9,strlen(strip_tags(innerHTML($Cell_nodes[4])))-10);
    		
    		$json_out[]=array("vegname"=>$sub_string[0],"market"=>$sub_string[1],"retail"=>$sub_string[2],"shopping"=>$sub_string[3]);
    	}
    	//header('Content-Type: application/json');
        if(count($json_out)>0){
          $json_out=json_encode($json_out);
          file_put_contents($json_file,$json_out);
          $con=getDatabaseConnection($state,$district);
          insertDataToDB($con,$json_out,$state,$district,$date);
          $con->close();
